...
resource "null_resource" "configure_server" {
  ...
  provisioner "local-exec" {
    working_dir = "../ansible/"
    command = "ansible-playbook --inventory '${aws_instance.myapp-server.public_ip}', --private-key ~/.ssh/id_rsa -u ec2-user ansible_python_interpreter=/usr/bin/python3 playbook.yaml"
  }
}
